        ...
        <article className="h-72 rounded-2xl border border-[var(--line)] bg-[var(--bg-panel-soft)] p-3 sm:h-80 sm:p-4">
          ...
          <ResponsiveContainer width="100%" height="88%">
            <BarChart data={metrics.weeklyProgress}>
              ...
              <YAxis />
              <Tooltip />
              <Bar dataKey="completed" fill="#d0ff28" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </article>
        ...
        <article className="h-72 rounded-2xl border border-[var(--line)] bg-[var(--bg-panel-soft)] p-3 sm:h-80 sm:p-4">
          ...
          <ResponsiveContainer width="100%" height="88%">
            <BarChart data={metrics.weightByExercise} layout="vertical" margin={{ left: 30 }}>
              ...
              <Tooltip />
              <Bar dataKey="avgWeight" fill="#25d6ff" radius={[0, 6, 6, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </article>
      </div>
        ...
        {metrics.discomfortNotes.length === 0 ? (
          ...
          <div className="space-y-2">
            {metrics.discomfortNotes.map((note, index) => (
              <p key={`${note.exercise}-${index}`} className="rounded-lg bg-[#0f141a] px-3 py-2 text-sm">
