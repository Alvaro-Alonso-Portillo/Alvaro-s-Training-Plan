  ...
  return (
    <section className="space-y-4 sm:space-y-6">
      <div className="grid gap-3 sm:gap-4 xl:grid-cols-2">
        <article className="h-72 rounded-2xl border border-[var(--line)] bg-[var(--bg-panel-soft)] p-3 sm:h-80 sm:p-4">
          ...
          <ResponsiveContainer width="100%" height="88%">
            <LineChart data={metrics.weeklyProgress}>
              <CartesianGrid strokeDasharray="3 3" stroke="#2a3644" />
              <XAxis dataKey="week" tick={{ fontSize: 11 }} />
              <YAxis />
              <Tooltip />
              <Line type="monotone" dataKey="volume" stroke="#d0ff28" strokeWidth={3} />
            </LineChart>
          </ResponsiveContainer>
        </article>
        ...
        <article className="h-72 rounded-2xl border border-[var(--line)] bg-[var(--bg-panel-soft)] p-3 sm:h-80 sm:p-4">
          ...
          <ResponsiveContainer width="100%" height="88%">
            <BarChart data={metrics.topRepeated}>
              ...
              <YAxis />
              <Tooltip />
              <Bar dataKey="count" fill="#25d6ff" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </article>
      </div>

      <article className="rounded-2xl border border-[var(--line)] bg-[var(--bg-panel-soft)] p-3 sm:p-4">
        ...
        {metrics.maxWeights.length === 0 ? (
          ...
          <div className="grid gap-2 md:grid-cols-2">
            {metrics.maxWeights.map((item) => (
              <p key={item.exercise} className="rounded-lg bg-[#0f141a] px-3 py-2 text-sm">
      ...
      <article className="grid grid-cols-2 gap-3 rounded-2xl border border-[var(--line)] bg-[var(--bg-panel-soft)] p-3 shadow-[inset_0_0_0_1px_rgba(208,255,40,0.08)] sm:p-4 md:grid-cols-4">
